...
RUN pip install pytest sentencepiece matplotlib jupyter ipython ipdb gpustat spacy munch opt_einsum fvcore gsutil cmake pykeops together_web3 together_worker diffusers[torch] \
    && python -m spacy download en_core_web_sm

# Core packages
RUN pip install torch==2.0.1 transformers==4.31.0 datasets==2.5.1 accelerate==0.21.0 \
    && torchvision==0.15.2 tokenizers==0.13.3 peft==0.4.0 \
